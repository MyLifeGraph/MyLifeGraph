-- Additive optional Skillset namespace. Existing branch validation, owner lock,
-- retry identities, projections, events and role grants remain unchanged.
-- Backend validation must be deployed before enabling new-client fields.
...
begin
  ...
  -- Older clients omit the additive namespace: keep its exact saved values.
  -- New clients send an explicit map, including null to clear an answer.
  if not (p_capture ? 'skillset') and current_capture ? 'skillset' then
    stored_capture := stored_capture || jsonb_build_object('skillset', current_capture -> 'skillset');
  end if;
  if next_capture_version = 'daily-capture-v5'
     and p_capture ->> 'branch_version' <> 'daily-capture-v5' then
    stored_capture := stored_capture || jsonb_build_object('compatibility', true);
  else
