...
select ok(
  ...
  'Coach V3 claim remains backend-only with a hardened fixed search path'
);
...
select is(
  ...
  'pending',
  'Coach V3 still creates a pending request'
);
...
select is(
  ...
  'failed|interrupted',
  'an expired Coach V3 replay still atomically records and returns interruption'
);
reset role;
