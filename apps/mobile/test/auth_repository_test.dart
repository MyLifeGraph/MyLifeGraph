...
void main() {
  ...
  group('webOAuthRedirectTo', () {
    test('uses a hosted web origin with a trailing slash', () {
      final redirectTo = webOAuthRedirectTo(
        Uri.parse('https://app.example.test/#/auth'),
      );

      expect(redirectTo, 'https://app.example.test/');
    });
  ...
  test(
    'mock and authenticated demo profiles never migrate guest check-ins',
    () {
  ...
  test(
    'authenticated local-demo reload overlays local setup name and state',
    () async {
  ...
  test(
    'profile participation comes from backend columns, not user metadata',
    () async {
      final requests = <http.Request>[];
      final client = SupabaseClient(
        'http://localhost:54321',
  ...
  test(
    'authenticated local-demo reload honors compatibility setup prefs',
    () async {
      SharedPreferences.setMockInitialValues({
  ...
  test(
    'local-demo onboarding state never falls through from a remote profile',
    () async {
      final overlaid = await overlayLocalDemoSetup(
        profile: _profile(
          email: 'demo@personal-coach.local',
  ...
  test(
    'hosted email auth fails closed and forwards a fresh CAPTCHA token',
    () async {
      final requests = <http.Request>[];
      final client = SupabaseClient(
        'http://localhost:54321',
      ...
      for (var index = 0; index < operations.length; index += 1) {
        ...
        expect(
          (body['gotrue_meta_security']
              as Map<String, dynamic>)['captcha_token'],
          token,
        );
      }
    },
  ...
  test(
    'failed signup and abandoned OAuth retain no participation consent',
    () async {
      final requests = <http.Request>[];
      final client = SupabaseClient(
        'http://localhost:54321',
