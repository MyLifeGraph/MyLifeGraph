...
| Source regression | Proves rejection paths, shared exact running/isolated image recognition, backup-before-reset order, target-drift refusal, exact `--local` invocation, absence of unsafe reset targets, and portable expected-error/archive classification without optional runner tools. | `scripts/test_local_supabase_migrations.sh` |
