...
volume. Expected lock-timeout, role-guard, Coach-limit, archive-entry, and
source-scan classification use the runner's baseline text tools and do not
depend on optional `rg`; another database in the normal cluster is not an
isolation boundary.
