...
Exact hosted `staging` and `pilot` sessions require the current backend-owned
`pilot-participation-v1` / `pilot-participation-notice-v1` version/time pair
before Setup only when `PILOT_PARTICIPATION_REQUIRED=true`. The current optional
small-pilot profile permits Setup without acceptance and creates no acceptance
record automatically. The explicit acceptance command remains bearer-derived,
stores no birth date, and does not trust editable Auth metadata. Neither mode
changes the canonical-profile requirement, Intake payload, Setup revision, or
Setup-owned materialization behavior.
