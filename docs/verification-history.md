...
This archive preserves the former verification runbook as historical context
and every dated local result unchanged as checkout evidence. Two obsolete
`<ref>` placeholders and the superseded CI workflow reference in the surrounding
runbook were normalized to the current safe references. This archive does not
define current commands, contracts, gaps, or pass claims. Use
`docs/verification.md` for all current verification decisions and evidence.

## Archived Runbook And Evidence
...
| Affected | `npm run verify:affected -- --base-ref <task-base-ref>` | Conservatively selects gates from all committed, working-tree, and untracked paths; core/Auth/routing/config/schema/mixed/unknown changes escalate to full. | Depends on selected gate |
...
The `docs-visual` job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml)
runs the same gate for pushes and pull requests with `DOCS_BASE_REF` set to the
event's base commit, so committed branch changes receive the same docs-impact
check.
...
`npm run verify:affected -- --base-ref <task-base-ref>`. CI failure artifacts are retained
