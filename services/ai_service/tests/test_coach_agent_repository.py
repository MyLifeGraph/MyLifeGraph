...
class Client:
    ...
    async def rpc(self, function, *, params):
        ...
        if function == "claim_coach_request_v3":
            return {
                "state": "pending",
...
def test_v3_claim_binds_only_request_identity_message_and_backend_provenance() -> None:
    client = Client()
    ...
    assert function == "claim_coach_request_v3"
    assert set(params) == {
        "p_user_id",
        "p_request_id",
