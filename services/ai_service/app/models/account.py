...
ACCOUNT_EXPORT_CONTRACT_VERSION = "account-export-v3"
...
class AccountExportLedgerPolicy(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_exact_v3_policy(self) -> "AccountExportLedgerPolicy":
        if tuple(self.sanitized_tables) != ACCOUNT_EXPORT_SANITIZED_TABLES:
            raise ValueError("sanitized_tables must match the V3 ledger policy")
        if self.omitted_tables != ACCOUNT_EXPORT_OMITTED_TABLES:
            raise ValueError("omitted_tables must match the V3 ledger policy")
        return self
...
class AccountExportLimits(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_exact_v3_limits(self) -> "AccountExportLimits":
        if (
            ...
            raise ValueError("limits must match the account-export-v3 contract")
        return self
...
class AccountExportResponse(BaseModel):
    ...
    contract_version: Literal["account-export-v3"]
    ...
    def validate_export_shape(self) -> "AccountExportResponse":
        ...
        if set(self.data) != expected_tables:
            raise ValueError("data must contain the exact V3 export table set")
        if set(self.record_counts) != expected_tables:
            raise ValueError("record_counts must contain the exact V3 table set")
        if any(
        ...
        if any(
            ...
            raise ValueError("record_counts exceed the V3 per-table bound")
        if sum(self.record_counts.values()) > ACCOUNT_EXPORT_MAX_TOTAL_ROWS:
            raise ValueError("record_counts exceed the V3 total bound")
        return self
